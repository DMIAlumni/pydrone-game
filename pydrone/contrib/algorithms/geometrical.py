...
from pydrone.algorithms import BaseAlgorithm
from pydrone.utils.direction_modifier import void_directions, get_direction, d


class GeometricAlgorithm(BaseAlgorithm):
    """
    Federico Dolce geometrical algorithm
    """

    def search_far_calibration(self, drone):
        ...
        return self.go_far(drone) if distances[-1] > 1.0 else self.search_close(drone)

    def go_far(self, drone):
        if len(drone.distances) > 3 and (drone.distances[-2] - drone.distances[-1]) < 1:
            if drone.last_modifier == 0:
                drone.last_modifier = 1 if random() < 0.5 else -1
    ...
    def change_strategy(self, drone):
        drone.distances = []
        close_distances = []
